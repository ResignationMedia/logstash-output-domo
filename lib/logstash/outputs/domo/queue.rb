  ...
  module Outputs
    class Domo
      class DomoQueue
        ...
        #
        # @return [DomoQueueJob, nil]
        def pop
          _, job = @client.blpop(@queue_name, 5)
          if job.nil?
            return nil
          end
          DomoQueueJob.from_json!(job)
        end

        # Push a DomoQueueJob to the queue
        #
        # @param job [DomoQueueJob] The job to be added.
        def push(job)
        ...
        # Iterate over all the jobs in the queue.
        # *This will call the #pop method!*
        #
        # @param &block [Proc]
        # @return [DomoQueueJob]
        def each(&block)
